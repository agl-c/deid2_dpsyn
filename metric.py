...
from config.path import DETAILED_SCORES_DIR
...
class TidyFormatKMarginalMetric:
    ...
    def overall_score_no_bias(self):
        # get the matrix of scores for each PUMA-YEAR (row) and k-col permutation (col)
        all_scores = self.k_marginal_scores()
        # take the row-wise mean to get the score per PUMA-YEAR
        scores = all_scores.mean(axis=1).ravel()
...
def score_online(
    ...
    for epsilon in epsilons:
        ...
        logger.info(f"starting calculation for epsilon={epsilon}")
        if detailed_scores:
            epsilon_score, detailed_scores = metric.overall_score_no_bias()
            # save detailed scores
            # np.savetxt(DETAILED_SCORES_DIR / f"{data_name}.csv", detailed_scores, delimiter=",")
        else:
            epsilon_score = metric.overall_score()

        logger.info(f"bias threshold:{bias_penalty_cutoff}")
        logger.info(f"{metric.bias_mask.sum()} PUMA-YEARs received a bias penalty")
        logger.success(f"score for epsilon {epsilon}: {epsilon_score}")
        scores_per_epsilon.append(epsilon_score)
    ...
    return mean_score, score_dict


if __name__ == "__main__":
    priv_data = 2
    syn_data = pd.read_csv(f'./dpsyn-0-state-{priv_data}-data.csv')
    truth_path = Path(f'./data/state-{priv_data}-data.csv')
    score_online(truth_path, syn_data)
