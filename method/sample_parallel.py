...
class Sample(DPSyn):
    ...
    def synthesize(self) -> pd.DataFrame:
        ...
        if scoring is None or scoring == 'pub':
            ...
            scoring_marginals = noisy_marginals
        ...
        if self.eps < self.sensitivity / 35:
            ...
            rounded_puma_year = (np.round(noisy_puma_year / 50) * 50).astype(int)
        else:
        ...
        # total_data = self.sample(init_data, scoring_marginals, weights, n=n, T=int(n / 3))
        # parallel
        syn_data_count = {}
        # for count_i, cell_count in enumerate(np.unique(rounded_puma_year)):
        #     logger.info(f'=========== ITER {count_i}, for count{cell_count} / {len(np.unique(rounded_puma_year))} =================')
        #     # sample_idx = np.random.choice(len(total_data), size=cell_count)
        #     tmp = self.sample(np.copy(total_data), scoring_marginals, weights, n=cell_count, T=int(cell_count / 50))
    ...
    def sample(self, init_data, target_marginals, weights, n, T) -> np.ndarray:
